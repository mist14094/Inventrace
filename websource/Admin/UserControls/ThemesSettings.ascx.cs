    ...
    public partial class ThemeSettingsControl : System.Web.UI.UserControl
    {
        ...
        protected void dlItems_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            if (item.ItemType == ListItemType.Item || item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                var btnDelete = ((ImageButton)item.FindControl("btnDelete"));
        ...
        //protected void bthAddTheme_Click(object sender, EventArgs e)
        //{
        //    MsgErr(true);

        //    if (!ThemeLoad.HasFile)
        //    {
        //        MsgErr(choosegMsg);
        //        return;
        //    }

        //    if (!ThemeLoad.FileName.EndsWith(".zip"))
        //    {
        //        MsgErr(extensionErrMsg);
        //        return;
        //    }

        //    string filename = string.Format("{0}/design/{1}s/{2}", designFolderPath, DesignType.ToString(), ThemeLoad.FileName);

        //    ThemeLoad.SaveAs(filename);
        ...
        private void InstallDesign(string url, string themeName)
        {
            try
            {
                ...
                {
                    string filename = string.Format("{0}design\\{1}s\\{2}.zip", designFolderPath, DesignType.ToString(), themeName);
        ...
        private void UninstallDesign(string themeName)
        {
            ...
            {
                _currentDesigns = DesignService.GetDesigns(DesignType);
                string dirname = string.Format("{0}design\\{1}s\\{2}", designFolderPath, DesignType.ToString(), themeName);

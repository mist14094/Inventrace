    ...
    public partial class OrderItemsControl : System.Web.UI.UserControl
    {
        ...
        protected void dlItems_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "SaveQuantity")
            {
                ...
                {
                    float quantity = 0;
                    if (e.Item.FindControl("txtQuantity") != null && float.TryParse(((TextBox)e.Item.FindControl("txtQuantity")).Text, out quantity))
                    {
                        if (quantity > 0)
                        {
                            OrderItems.Find(oi => oi.OrderItemID == SQLDataHelper.GetInt(e.CommandArgument)).Amount = quantity;
                        }
                        else
                        {
        ...
        protected void btnAddProductByArtNo_Click(object sender, EventArgs e)
        {
            ...
            else if (txtArtNo.Text.IsNotEmpty())
            {
                ...
                product = ProductService.GetProductByOfferArtNo(searchTerm);
                if (product == null)
                {

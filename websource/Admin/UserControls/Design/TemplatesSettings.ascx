...
<asp:Label ID="lTrialMode" runat="server" Text="<%$ Resources:Resource, Admin_Design_TrialMode %>" ForeColor="Red"></asp:Label>
<br />
<br />

<asp:ListView ID="DataListTemplates" runat="server" OnItemCommand="dlItems_ItemCommand" ItemPlaceholderID="itemPlaceholderId">
    <LayoutTemplate>
    ...
    <ItemTemplate>
            <div class="template-admin-item <%# (Convert.ToBoolean(Eval("IsInstall")) && SettingsDesign.Template == Convert.ToString(Eval("StringId"))) ? "current" : "" %>">
                <%# RenderTemplatePicture((string)Eval("PreviewImage"), (string)Eval("StringId"))%>
                ...
                <div>
                    <div class="left">
                        <asp:LinkButton CssClass="template-admin-apply" ID="btnActivate" runat="server" CommandArgument='<%# Eval("StringId") %>' CommandName="ApplyTheme"
                            Visible='<%# Convert.ToBoolean(Eval("IsInstall")) && Convert.ToBoolean(Eval("Active")) && SettingsDesign.Template != Convert.ToString(Eval("StringId")) %>'>
                        ...
                        <asp:HyperLink ID="btnSettings" runat="server" NavigateUrl="~/Admin/TemplateSettings.aspx"
                            Text='<%$ Resources:Resource, Admin_ThemesSettings_Settings %>'
                            Visible='<%# Convert.ToBoolean(Eval("IsInstall")) && SettingsDesign.Template == Convert.ToString(Eval("StringId")) %>' />

                        <asp:HyperLink runat="server" ID="lbBuy" CssClass="addbtn showtooltip"
                            NavigateUrl='<%# Eval("DemoLink") %>' Target="_blank"
                            Text='<%$ Resources:Resource, Admin_ThemesSettings_BuyTheme %>'
                            ToolTip='<%$ Resources:Resource, Admin_ThemesSettings_BuyTheme %>'
                    ...
                    <div class="right">                        
                        <asp:LinkButton runat="server" ID="btnAdd" CausesValidation="false" CommandArgument='<%# Eval("id") %>'
                            CommandName="Add" CssClass="addbtn showtooltip themeadd"
                            Text='<%$ Resources:Resource, Admin_ThemesSettings_Add %>'
                        ...
                        <asp:LinkButton runat="server" ID="btnDelete" CausesValidation="false" CommandArgument='<%# Eval("StringId") %>'
                            CommandName="Delete" CssClass="deletebtn showtooltip themedel"
                            Text='<%$ Resources:Resource, Admin_ThemesSettings_Delete %>'

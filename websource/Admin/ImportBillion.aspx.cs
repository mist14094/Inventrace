...
namespace Admin
{
    public partial class ImportBillion : AdvantShopAdminPage
    {
        ...
        {
            if (CommonStatistic.ThreadImport.IsAlive)
            {
        ...
        private Int32 ProcessProduct(Random rnd, Int32 intProductIndex)
        {
            ...
            if (!CommonStatistic.IsRun) return 0;

            var product = new Product();

            // Simple prop

            product.ArtNo = Strings.GetRandomString(rnd, 50, -1);
                ...
                var offr = new Offer
                    {
                        ...
                        ArtNo = Strings.GetRandomString(rnd, 50, -1),
                        ColorID = Colors[DevTool.GetRandomInt(rnd, 0, Colors.Count - 1)].ColorId,
                        SizeID = Sizes[DevTool.GetRandomInt(rnd, 0, Sizes.Count - 1)].SizeId,
                    };


                product.Offers.Add(offr);
            }

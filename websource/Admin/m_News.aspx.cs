...
namespace Admin
{
    public partial class m_News : AdvantShopAdminPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            var modules = AttachedModules.GetModules(AttachedModules.EModuleType.SendMails);

            var modulesAny = false;
            foreach (var moduleType in modules)
            {
                var moduleObject = (ISendMails)Activator.CreateInstance(moduleType, null);
                if (ModulesRepository.IsInstallModule(moduleObject.ModuleStringId))
                {
                    modulesAny = true;
                }
            }

            if (!modulesAny) return;

            tableMailChimpNotActive.Visible = false;
            tableSendMailTypes.Visible = true;
        }

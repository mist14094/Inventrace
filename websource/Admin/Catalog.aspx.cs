...
namespace Admin
{
    public partial class Catalog : AdvantShopAdminPage
    {
        ...
        protected void lbDeleteSelected1_Click(object sender, EventArgs e)
        {
            if (!_inverseSelection)
            {
                if (_selectionFilter != null)
                {
                    ...
                    {
                        CategoryService.DeleteCategoryAndPhotos(SQLDataHelper.GetInt(catId));
                    }
                }
            }
            else
                ...
                default:
                    if (!_inverseSelection)
                    {
                        ...
                        if (_selectedCategories != null)
                        {
                            ...
                            {
                                CategoryService.DeleteCategoryAndPhotos(SQLDataHelper.GetInt(id));
                                CategoryService.DeleteCategoryLink(SQLDataHelper.GetInt(id));
                            }
                        }
                    }
                    else
                    {
                        foreach (var p in CategoryService.GetProductsByCategoryId(CategoryId))
                        {
                            ...
                            {
                                CategoryService.DeleteCategoryAndLink(p.ProductId, SQLDataHelper.GetInt(Request["categoryid"]));
                            }
                        }
        ...
        #region ѕеренос товаров в другую категорию

        protected void btnChangeProductCategory_Click(object sender, EventArgs e)
        {
            // ѕеренос продуктов в другую категорию
        ...
        /// <summary>
        /// ѕеренос выбранных товаров в другую категорию
        /// </summary>
        private void ChangeProductsCategory()
        {
            int categoryId = SQLDataHelper.GetInt(tree2.SelectedValue);

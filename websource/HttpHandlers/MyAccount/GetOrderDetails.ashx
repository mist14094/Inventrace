...
public class GetOrderDetails : IHttpHandler, IRequiresSessionState
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (order.OrderCustomer == null ||
            order.OrderCustomer.CustomerID != CustomerSession.CurrentCustomer.Id)
        {
            context.Response.Write(JsonConvert.SerializeObject(string.Empty));
            return;
        }
        ...
        {
            var allPaymentstemp = PaymentService.GetAllPaymentMethods(true);
            ...
            allPayments = UseGeoMapping(allPayments, order.BillingContact.Country, order.BillingContact.City);
        ...
        var orderDetails = new
                       {
                           ...
                           PaymentPrice = order.PaymentCost != 0 ? CatalogService.GetStringPrice(order.PaymentCost, order.OrderCurrency.CurrencyValue, order.OrderCurrency.CurrencyCode) : string.Empty,

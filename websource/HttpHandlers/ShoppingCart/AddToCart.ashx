...
namespace HttpHandlers.ShoppingCart
{
    public class AddToCart : IHttpHandler, IRequiresSessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            {
                product = ProductService.GetProduct(context.Request["productid"].TryParseInt());
            ...
            foreach (var moduleShoppingCartPopup in AttachedModules.GetModules(AttachedModules.EModuleType.ShoppingCartPopup))
            {
                ...
                if (classInstance.CheckAlive() && classInstance.Active() && context.Request.UrlReferrer!= null && !context.Request.UrlReferrer.ToString().Contains("/social/"))
                {

...
namespace AdvantShop.Catalog
{
    public class CategoryService
    {
        ...
        {
            List<CatalogItem> res = SQLDataAccess.ExecuteReadList("[Catalog].[sp_GetCategoryContent]", CommandType.StoredProcedure,
        ...
        public static IList<Product> GetProductsByCategoryId(SQLDataAccess db, int categoryid, bool inDepth)
        {
            ...
            db.cmd.CommandText = inDepth ? "SELECT * FROM [Catalog].[Product] INNER JOIN [Catalog].[ProductCategories] on ProductCategories.ProductID = Product.ProductID WHERE [ProductCategories].CategoryID  IN (SELECT id FROM [Settings].[GetChildCategoryByParent](@categoryId)) AND [Product].[Enabled] = 1  AND [Product].[CategoryEnabled] = 1"
        ...
        public static int AddCategory(Category cat, bool updateCache, SQLDataAccess db)
        {
            ...
            if (updateCache)
            {
                ...
                foreach (var category in GetChildCategoriesByCategoryId(0, true))
                {
                    ...
                    CacheManager.Remove("MenuCatalog_Alternative" + category.CategoryId);
                }
                CacheManager.Remove("MenuCatalog0");
                CacheManager.Remove("MenuCatalog_Alternative0");
        ...
        private static IEnumerable<int> DeleteCategory(int categoryId, bool updateCache)
        {
            ...
            if (updateCache)
            {
                ...
                foreach (var cat in GetChildCategoriesByCategoryId(0, true))
                {
                    ...
                    CacheManager.Remove("MenuCatalog_Alternative" + cat.CategoryId);
                }
        ...
        {
            var c = SQLDataAccess.ExecuteReadOne("SELECT TOP (1) [CategoryID], [Name], [Picture], [ParentCategory], [Products_Count] FROM [Catalog].[Category] WHERE ParentCategory = 0 AND CategoryID <> 0 ORDER BY [SortOrder], [CategoryID]",
        ...
        public static bool UpdateCategory(Category category, bool updateCache)
        {
            ...
            if (updateCache)
            {
                ...
                CacheManager.Remove("MenuCatalog0");
                CacheManager.Remove("MenuCatalog_Alternative");
        ...
        public static string GetCategoryStringByProductId(int productId, bool onlySort = false)
        {
            ...
            if (onlySort)
            {
                var resSort = categoryDict.Select(
                    pair =>
                    String.Format("{0}", pair.Value)).AggregateString('-');
                return resSort;
            }
        ...
        public static void SubParseAndCreateCategory(string strCategory, int productId, string sorting = "")
        {
            //
            ...
            string[] sortCat = sorting.Split(new[] { "-" }, StringSplitOptions.RemoveEmptyEntries);
            ...
            foreach (string categoryString in cats)
            {
                ...
                //var categoryPath = categoryString.Trim(new[] { '[', ']' });
                ...
                foreach (string categoryName in categories)
                {
                        ...
                        parentId = AddCategory(new Category
                            {
                                ...
                                UrlPath = UrlService.GetEvalibleValidUrl(0, ParamType.Category, categoryName),
                    ...
                    {
                        ProductService.AddProductLink(productId, parentId, productSortOrder);

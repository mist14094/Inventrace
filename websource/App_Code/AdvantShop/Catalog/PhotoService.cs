    ...
    public class PhotoService
    {
        ...
        public static Photo GetMainProductPhoto(int productId, int? colorID = null)
        {
            ...
            return SQLDataAccess.ExecuteReadOne<Photo>(
                string.Format(
                    "SELECT top 1 * FROM [Catalog].[Photo] WHERE [objId] = @productId and type=@type {0} ORDER BY main desc, [PhotoSortOrder]",
        ...
        public static IEnumerable<string> GetNamePhotos(int objId, PhotoType type, bool allNames)
        {
            if (allNames)
        ...
        public static bool IsProductHaveThisPhotoByName(int productId, string originName)
        {
            ...
            return name.IsNotEmpty() && File.Exists(FoldersHelper.GetImageProductPathAbsolut(ProductImageType.Original, name));
        }

        public static void UpdateProductPhoto(Photo ph)
        {
            SQLDataAccess.ExecuteNonQuery("Update Catalog.Photo set Description=@Description, PhotoSortOrder = @PhotoSortOrder, ColorID=@ColorID Where PhotoID = @ProductPhotoID",
                                            CommandType.Text,
                                            new SqlParameter("@ProductPhotoID", ph.PhotoId),
                                            new SqlParameter("@PhotoSortOrder", ph.PhotoSortOrder),
        ...
        {
            foreach (var photoName in GetNamePhotos(objId, type, false))
            {
        ...
        public static string PhotoToString(List<Photo> productPhotos)
        {
            ...
            foreach (var photo in productPhotos.OrderBy(ph => ph.ColorID))
            {
                ...
                sb.Append(photo.PhotoName + (color != null ? ":" + color.ColorName : "") + ",");
            }
            ...
            //int? currentColorID = null;

            //foreach (var photo in productPhotos.OrderBy(ph=>ph.ColorID))
            //{
            //    if (currentColorID != photo.ColorID)
            //    {
            //        currentColorID = photo.ColorID;
            //        var currentColor = ColorService.GetColor(currentColorID);
            ...
            //        {
            //            strb.Append(currentColor.ColorName + ":");
            //        }
            //    }
            //    strb.Append(photo.PhotoName + ",");
            //}

            //return strb.ToString();
        }

        public static void PhotoFromString(int productId, string photos)
        {

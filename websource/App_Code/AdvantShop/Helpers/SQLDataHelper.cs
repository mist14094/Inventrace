...
namespace AdvantShop.Helpers
{
    public class SQLDataHelper
    {
        ...
        {
#if !DEBUG
            if (!HasColumn(rdr, columnName)) return defaultValue;
#endif
        ...
        {
#if !DEBUG
            if (!HasColumn(rdr, columnName)) return null;
#endif
            int index = rdr.GetOrdinal(columnName);
            if (rdr.IsDBNull(index))
            {
        ...
        {
#if !DEBUG
            if (!HasColumn(rdr, columnName)) return decimal.Zero;
#endif
        ...
        {
#if !DEBUG
            if (!HasColumn(rdr, columnName)) return double.MinValue;
#endif
        ...
        {
#if !DEBUG
            if (!HasColumn(rdr, columnName)) return 0F;
#endif
        ...
        {
#if !DEBUG
            if (!HasColumn(rdr, columnName)) return Guid.Empty;
#endif
        ...
        {
#if !DEBUG
            if (!HasColumn(rdr, columnName)) return int.MinValue;
#endif

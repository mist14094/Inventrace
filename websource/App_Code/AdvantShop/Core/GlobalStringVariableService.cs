...
namespace AdvantShop.Core
{
    public class GlobalStringVariableService
    {
        ...
        public static string TranslateExpression(string str, MetaType type, string name, string categoryName = null, string brandName = null)
        {
            var dict = (Dictionary<string, string>)GetGlobalVariables().AddRange(GetSpecificVariables(type, name, categoryName, brandName));
            return TranslateExpression(str, dict);
        }

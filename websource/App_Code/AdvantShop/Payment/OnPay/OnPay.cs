...
namespace AdvantShop.Payment
{
    public class OnPay : PaymentMethod
    {
        ...
        private string GetCheckResponseXML(HttpRequest req)
        {
            ...
            sb.AppendLine("<md5>" + string.Format("check;{0};{1};{2};{3};{4}", req["pay_for"], req["order_amount"], req["order_currency"], 0, SecretKey).Md5(false) + "</md5>");
            sb.AppendLine("</result>");
            return sb.ToString();
        }
        ...
        private string GetPayResponseXML(int iCode, int pay_for, string order_amount, string onpay_id, string comment, string order_currency)
        {
            ...
            sb.AppendLine("<md5>" + string.Format("pay;{0};{1};{2};{3};{4};{5};{6}", pay_for, onpay_id, pay_for, order_amount, order_currency, iCode, SecretKey).Md5(false) + "</md5>");
            sb.AppendLine("</result>");
            return sb.ToString();
        }

    ...
    public class StatisticService
    {
        ...
        public static int GetOrdersCountByDateRange(DateTime fromDate, DateTime toDate)
        {
            return Convert.ToInt32(SQLDataAccess.ExecuteScalar(
                "SELECT COUNT(OrderID) FROM [Order].[Order] inner join [Order].[OrderStatus] ON [OrderStatus].OrderStatusID = [Order].OrderStatusID WHERE [OrderStatus].[IsCanceled] = 0 and [OrderDate] IS NOT NULL AND Convert(date, @FromDate) <= Convert(date, [OrderDate]) AND Convert(date, [OrderDate]) <=  Convert(date, @ToDate)",
                CommandType.Text,
        ...
        {
            return Convert.ToInt32(SQLDataAccess.ExecuteScalar(
                "SELECT COUNT(OrderID) FROM [Order].[Order] inner join [Order].[OrderStatus] ON [OrderStatus].OrderStatusID = [Order].OrderStatusID WHERE [OrderStatus].[IsCanceled] = 0 and [OrderDate] IS NOT NULL AND Convert(date, [OrderDate]) = Convert(date, @Date)",
                CommandType.Text,
                new SqlParameter("@Date", date)));
        }
        ...
        #region GoogleAnalitics statistic

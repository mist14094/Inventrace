...
public partial class _default : Page
{
    ...
    public ActiveDiv CurrentDiv
    {
        get
        {
            ...
            if (string.IsNullOrEmpty(Request["step"]))
            {
                ...
                _currentDiv = Trial.IsTrialEnabled ? ActiveDiv.TrialSelect : ActiveDiv.Shopinfo;
                return _currentDiv;
            }
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        MenuItems.AddRange(new List<MenuItem>{
                                              new MenuItem {MenuName=Resource.Install_Default_LeftMenu_AboutShop, Div=ActiveDiv.Shopinfo, StyleClass="shop-info"},
                                              new MenuItem {MenuName=Resource.Install_Default_LeftMenu_Finance, Div=ActiveDiv.Finance, StyleClass="finance"},
                                              new MenuItem {MenuName=Resource.Install_Default_LeftMenu_Payment, Div=ActiveDiv.Payment, StyleClass="payment"},
                                              new MenuItem {MenuName=Resource.Install_Default_LeftMenu_Shipping, Div=ActiveDiv.Shipping, StyleClass="shipping"},
                                              new MenuItem {MenuName=Resource.Install_Default_LeftMenu_Security, Div=ActiveDiv.OpenidParagraf, StyleClass="openid paragraf"},
                                              new MenuItem {MenuName=Resource.Install_Default_LeftMenu_Notification, Div=ActiveDiv.NotifyParagraf, StyleClass="notify paragraf"},
                                              new MenuItem {MenuName=Resource.Install_Default_LeftMenu_FinalStep, Div=ActiveDiv.Final, StyleClass="final"}
                                          });
    }
    ...
    protected string GetLeftMenu()
    {
        ...
        for (int i = 0; i < MenuItems.Count; i++)
        {
            if (MenuItems[i].Div < CurrentDiv)
                strHtml.Append(string.Format(tempalte, " post", MenuItems[i].StyleClass, MenuItems[i].MenuName, "location.href = '" + UrlService.GetAbsoluteLink("install/default.aspx") + "?step=" + MenuItems[i].Div + "'"));
            else if (MenuItems[i].Div == CurrentDiv)
                strHtml.Append(string.Format(tempalte, " active", MenuItems[i].StyleClass, MenuItems[i].MenuName, "return false;"));
            else
                strHtml.Append(string.Format(tempalte, string.Empty, MenuItems[i].StyleClass, MenuItems[i].MenuName, "return false;"));
        }
    ...
    protected void NextClick(object sender, EventArgs e)
    {
        if (CurrentDiv != ActiveDiv.Final)
        {
            ...
            temp++;
            Response.Redirect(UrlService.GetAbsoluteLink("install/default.aspx") + "?step=" + ((ActiveDiv)temp).ToString());
        }
    }

...
public partial class UserControls_ProductView : UserControl
{
    ...
    protected string RenderPictureTag(string urlPhoto, string additionalUrlPhoto, string urlPath, string photoDesc, string productName)
    {
        ...
        switch (ViewMode)
        {
            case SettingsCatalog.ProductViewMode.Tiles:
                strFormat = string.Format("<a href=\"{0}\"><img src=\"{1}\" alt=\"{2}\" class=\"scp-img p-photo\" /></a>", urlPath,
                                          FoldersHelper.GetImageProductPath(ProductImageType.Small, additionalUrlPhoto.IsNotEmpty() ? additionalUrlPhoto : urlPhoto, false), HttpUtility.HtmlEncode(alt));
                break;
    ...
    protected string RenderPriceTag(float price, float discount, float multiPrice)
    {
        if (multiPrice == 0)
        {
            return CatalogService.RenderPrice(price, discount, false, customerGroup, isWrap: true);
        }
        else
        {
            return Resources.Resource.Client_Catalog_From + " " + CatalogService.RenderPrice(price, discount, false, customerGroup, isWrap: true);
        }
    }
